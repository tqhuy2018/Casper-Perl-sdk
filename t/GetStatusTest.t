...
sub getStatus {
	...
	ok(length($getStatusResult->getApiVersion()) > 0, "Test api_version, Passed");
	ok($getStatusResult->getChainspecName() eq "casper-test", "Test chainspec_name, Passed");
	ok(length($getStatusResult->getStartingStateRootHash()) > 0, "Test starting_state_root_hash, Passed");
	...
	ok(length($getStatusResult->getBuildVersion()) > 0, "Test build_version, Passed");
