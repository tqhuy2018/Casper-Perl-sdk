...
sub getDeploy2 {
	...
	# Assertion for first Arg
	ok($oneNASession->getItsName() eq "recipient", "Test session 9th arg name = recipient - Passed");
	my $sessionArgCLValue = $oneNASession->getCLValue();
	...
	ok($oneNASession1->getItsName() eq "token_ids", "Test session 9th arg name = recipient - Passed");
...
sub getDeploy3 {
	...
	my $sessionArgCLValue1 = $oneNASession1->getCLValue();
	ok($sessionArgCLValue1->getBytes() eq "010000000102","Test session 5th arg CLValue, bytes value = 010000000102 - Passed");
